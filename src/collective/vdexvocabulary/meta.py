...
def base_directive(_context, cls, filename=None, directory=None,
                   environment=None, default_language=None):
    ...
    if filename:
        ...
        for filename in os.listdir(directory):
            if filename.endswith('.vdex'):
                registerfile(
                    _context, os.path.join(directory, filename),
                    cls, default_language=default_language
                )


def VdexVocabularyDirective(_context, file=None, directory=None,  # nopep8
                            environment=None, default_language=None):  # nopep8
